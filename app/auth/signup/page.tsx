...
export default function SignupPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    registerMutation.mutate(payload, {
      onSuccess: ({ data }) => toast.success(data.message || "Registration successful"),
      onError: (err: any) => toast.error(err.error || "Something went wrong"),
    });
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors flex items-center justify-center p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <motion.div
          ...
          className="hidden lg:block stack-lg"
        >
          <div className="flex items-center gap-3 mb-10">
            <div className="w-12 h-12 rounded-xl bg-gradient-primary flex items-center justify-center font-bold text-background text-3xl">S</div>
            <span className="font-bold text-3xl tracking-tight">{`Sutorium`}</span>
          </div>
        ...
        <motion.div
          ...
          className="w-full max-w-md card-clean p-10 rounded-xl border border-border mx-auto"
        >
          <Link
            href="/"
          ...
          <p className="text-muted-foreground mb-8">Join 2,000+ creators today.</p>
